...
namespace BuberDinner.Application.IntegrationTests
{
    ...
    public class TestFixture : IAsyncLifetime
    {
        ...
        // var Dinner = await _fixture.ExecuteDbContextAsync(dbCxt => dbCxt.Dinners.Where(ro => c.OrderId == 123).FirstOrDefaultAsync());
        public Task<T> ExecuteDbContextAsync<T>(Func<MMTDbContext, Task<T>> action)
        {
            return ExecuteScopeAsync(sp => action(sp.GetService<MMTDbContext>()));
        }
